...
class BlogMediaController extends Controller
{
    public function postBlogMedia(Request $request)
    {
        ...
        if ($request->hasFile('mediaVideo')) {
            $type = $request->file('mediaVideo')->getClientOriginalExtension();
            if ($type == 'mp4' ||  $type == 'mov' || $type == 'wmv' || $type == 'avi' || $type == 'x-m4v' || 
                $type == 'webm' || $type == 'x-ms-wmv' || $type == 'x-msvideo' || $type == '3gpp' || $type == 'mpeg' || 
